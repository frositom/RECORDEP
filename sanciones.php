...
// Connect to MSSQL
$link = mssql_connect($server, '8559', 'hack8');
...
// $link = mssql_connect('192.168.1.2', '8559', 'hack8');
 
// if (!$link)
    // die('No se logro conectar con el servidor');
 
// if (!mssql_select_db('BKRECORD', $link))
 ...
 // if ($result){   //si el resultado no viene vacío hace la instruccion
		...
		// if($x>0){
			...
			// foreach($result_array as $row){
				...
				// echo "usuario= ".$row['seg_usuario']." nombre= ".$row['seg_nombres']."<br>";   
